...
Поскольку пользователями приложения является ограниченный круг лиц, систему не нужно прорабатывать с точки зрения отказоустойчивости под высокой нагрузкой.
...
В качестве языка программирования выбран Python, а фреймворк - Django.

На этом варианте и будет сконцентрирован следующий [ADR](adr_1.md).